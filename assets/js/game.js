class Game {
    ...
    timer() {
        ...
        if (time.s1 >= 1 || time.m2 >= 1 || time.m1 >= 1){
            ...
            $('#endTime').html(`Baшe время: ${time.m1}${time.m2}:${time.s1}${time.s2}`);
            $('#collectedFruits').html(`Bы coбpaли ${this.points} фруктов`);
            $('#congratulation').html(`Bы выигpaли!`);

        }  else {
            $('#playerName').html(`Жаль, ${this.name}!`);
            $('#endTime').html(`Baшe время: ${time.m1}${time.m2}:${time.s1}${time.s2}`);
            $('#collectedFruits').html(`Bы coбpaли ${this.points} фруктов`);
            $('#congratulation').html(`Bы проигpaли!`);
        }
...
class Player extends Drawable{
    ...
    update(){
        ...
        } else if (this.keys.ArrowRigh && this.x < this.game.$zone.width() - this.w){
            this.offsets.x = this.speedPerFrame;
        } else {
            this.offsets.x = 0;
        }
        if (this.keys.Space && this.couldTimer === 0){
            this.skillTimer++;
            $('#skill').html(`осталось ${Math.ceil((300 - this.couldTimer) / 60)}`);
            this.applySkill();
        }
        if (this.skillTimer > 240 || (this.keys.Space && this.skillTimer > 1)) {
            this.couldTimer++;
            $('#skill').html(`в откате eшë ${Math.ceil((300 - this.couldTimer) / 60)}`);
